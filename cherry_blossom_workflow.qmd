---
title: "Cherry Blossom Modeling Workflow"
format: html
execute:
  echo: true
---
...
We will construct a series of datasets using the sites, temperature data from NASA Power, Niña anomaly data from NOAA, and aggregated chill hours. We utilize a basic linear model to detrend the data. Later, we will train our predictor to predict the residuals from the linear model.
...
### Introduce our model class

We leverage dependency injection in a general neural network Model class so that we don't have to rewrite training loops. Here we will also make sure that our Quarto document can run reticulate and some required python packages.
...
### Train our climate LSTM model

Train a stacked LSTM to predict the next daily extremes. We will use the latent representations of this model as features in the predictor model. Here are are using a relatively simple architecture with shallow linear layers encoding/decoding linear output.
...
Now we will construct the latent representations of of time series so that we only need to use the above neural network once in the whole workflow. We will also combine some of the other files we produced originally (like the chill hour statistics).
...
### Predict cherry blossom dates

Now we introduce the cherry blossom predicting model. This takes in the latent representations from the climate LSTM and pools them using exponentially decaying weights so that more recent representations contribute more. We then feed it into a simple residual neural network and produce a prediction for the linear model residuals.
...
Now we train that model on our assembled dataset.
...
Finally, we run the cities of interest through our model and add the 2026 projections from the linear model.
